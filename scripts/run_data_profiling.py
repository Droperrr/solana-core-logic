#!/usr/bin/env python3
"""
Скрипт для комплексного профилирования данных (Фаза 3.1)
...
def analyze_basic_stats(conn):
    ...
    if 'transactions' in tables:
        ...
        print(f"  Успешные транзакции: {result['success_rate']:.1f}%")
...
def analyze_tokens(conn):
    ...
    SELECT 
        ...
        AVG(CASE WHEN error IS NULL THEN 1 ELSE 0 END) * 100 as success_rate
    FROM transactions 
    ...
    for i, token in enumerate(tokens, 1):
        ...
        print(f"    Успешность: {token['success_rate']:.1f}%")
...
def analyze_ml_events(conn):
    ...
    SELECT 
        ...
        COUNT(DISTINCT from_wallet) as unique_from_wallets,
        COUNT(DISTINCT to_wallet) as unique_to_wallets
    ...
    print_subsection("Типы событий")
    for event in events[:10]:  # Топ-10
        print(f"  {event['event_type']} ({event['platform']}): {event['event_count']:,} событий")
    ...
    print_subsection("Общее распределение типов событий")
    for event_type in event_types:
        print(f"  {event_type['event_type']}: {event_type['total_count']:,}")
...
def analyze_wallet_roles(conn):
    ...
    if has_wallets:
        ...
        # Пытаемся анализировать через wallet_tag в ml_ready_events
        cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='ml_ready_events';")
        if cursor.fetchall():
            query = """
            SELECT 
            ...
            if tags:
                print_subsection("Теги кошельков (из ml_ready_events)")
                for tag in tags:
                    print(f"  {tag['wallet_tag']}: {tag['event_count']:,} событий")
            else:
                print("Нет данных о тегах кошельков")
...
def create_summary_report(conn):
    ...
    cursor.execute("SELECT AVG(CASE WHEN error IS NULL THEN 1 ELSE 0 END) * 100 as rate FROM transactions;")
    metrics['success_rate'] = cursor.fetchone()['rate']
    ...
    if metrics['success_rate'] > 90:
        ...
        print("  ⚠️  Умеренное качество данных")
